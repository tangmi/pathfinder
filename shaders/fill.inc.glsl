...
vec4 computeCoverage(vec2 from, vec2 to, texture2D uAreaLUT, sampler uAreaLUTSampler) {
    ...
    return texture(sampler2D(uAreaLUT, uAreaLUTSampler), vec2(y + 8.0, abs(d * dX)) / 16.0) * dX;
}
